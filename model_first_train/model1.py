...
#     '53병동': 5, '54병동': 5, '71병동': 7, '72병동': 7, '75병동': 7,
#     '76병동': 7, '78병동': 7, '83병동': 8, '69병동': 6,
#     '응급중환자실': 7, '뇌졸중집중치료실': 4
# }
...
#     ('I20','심혈관계중환자실'):0.1768, ('I20','54병동'):0.0656, ('I20','외과ICU'):0.0410,
...
#         total=('total_beds', 'first'),
#         occupied=('useSckbCnt', 'sum')
#     ).reset_index()
...
#         state = make_state_from_df(df_live)
#     else:
#         state = {w: {'total': WARD_TOTALS.get(w, 0), 'occupied': 0} for w in WARD_TOTALS}

#     scores = {}
#     candidates = self.edges.get(icd, [])

#     for w in candidates:
#         # 실시간 병상 정보가 없으면 기본 total 사용
#         if w not in state:
#             state[w] = {'total': WARD_TOTALS.get(w, 0), 'occupied': 0}

#         score = self.combined_score(icd, w, state)
#         scores[w] = score

#     # 추천 점수가 있으면 → 정렬된 top_k 반환
#     ranked = sorted(scores.items(), key=lambda x: x[1], reverse=True)
#     if ranked:
#         return ranked[:top_k]

#     # 추천 점수가 없다면 → fallback: 순서만 반영
#     fallback = self.edges.get(icd, [])[:top_k]
#     return [(w, 0.0) for w in fallback]
#hybrid_scheduler.py
from collections import defaultdict
import pandas as pd
